...
export default function CrashMarker({onDrag, pin, draggable}: {onDrag ?: (e: google.maps.MapMouseEvent) => void, pin: CrashData,  draggable: boolean}){
    ...
    const severityScalingMap :{[key: string]: number}=  {
      "f": 1.25,
      "s": 1,
      "m": 0.85,
      "p": 0.75
    ...
    return (
        <AdvancedMarker zIndex={severityZIndexMap[pin.severity]} clickable={true} onClick={onDrag === undefined ? handleMarkerClick: undefined} ref={markerRef} onDragEnd={onDrag} draggable={draggable} key={pin.id} position={{lat: pin.position.latitude, lng: pin.position.longitude}}>
            <span onClick={onDrag === undefined ? handleMarkerClick: undefined}>
            <Pin scale={severityScalingMap[pin.severity]} background={severityColorMap[pin.severity]} borderColor={severityColorMap[pin.severity]} glyph={pin.severity === "s" || pin.severity === "f" ? pinSvg: minorSvg}/>
            </span>
            {
